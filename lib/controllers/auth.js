...
exports.test = (req, res, next) => {
	// next for static
	if (utils.isStatic(req)) return next();

	req.options.log.info('Start test auth', req.originalUrl);
	...
	if (
		...
		let back =  req.originalUrl;
		req.options.log.warn('Start test auth bad. Redirect to auth', 'back to ' + back);

		if (back !== req.options.home)
			res.cookie('back', back,  { maxAge: myUtil.date.ts.DAY, path: req.options.baseUrl});
...
/**
 * Render login page. Check post request
 * @param {object}req
 * @param {object}res
 * @param {function}next
 * @returns {*}
...
exports.render = (req, res, next) => {
	...
	let data = utils.renderData(req,{
		login: login,
		isBad : req.method === 'POST',
		isProp : process.env.NODE_ENV !== 'dev'
	});

	req.options.log.info('Auth render get');

	res.render(__dirname + '/../views/auth-content.html', data);
};
