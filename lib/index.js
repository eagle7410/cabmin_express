...
exports.init = (express, app, opt, cb) => {
	...
	utils.forObj(router.options.users, (login , data) => data.password && !data.hash ?
		router.options.users[login].hash = utils.sha512(data.password) :
		null
	);
